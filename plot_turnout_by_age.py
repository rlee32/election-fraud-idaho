...
def get_voters() -> Dict[str, List[any]]:
    ...
    with open(DATA_FILE, 'r') as f:
        ...
        for row in reader:
            header = row
            break
        county_index = header.index('ResCountyDesc')
        ...
        for row in reader:
            ...
            if not r:
                ...
                assert(vid not in reg_errors)
                reg_errors.add(vid)
                continue
            if r == '12:00:00 AM':
                print(f'found wierd registration date: {r}')
                assert(vid not in reg_errors)
                reg_errors.add(vid)
                continue
            ...
            age = row[age_index]
            if not age:
                print(f'skipping no age: {row}')
                assert(vid not in age_errors)
                age_errors.add(vid)
                continue
            age = int(age)
            if age < 18:
        ...
        print(f'found {len(age_errors)} age errors.')
        print(f'found {len(reg_errors)} registration errors.')
